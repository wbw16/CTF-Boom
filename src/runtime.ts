...
/**
 * Route one OpenCode event for a root conversation. Child sessions created by `task` share the
 * root's engagement and cancellation tree. Their visible text/tool events stay inside the task
 * result, while their step usage is remapped to the root so Boom's run budget and consumption
 * include every worker. OpenCode reports cost per step; the router exposes its running tree total
 * because the provider-neutral turn reconciler treats event cost as a cumulative snapshot.
 */
export function createOpenCodeConversationEventRouter(
  rootSessionID: string,
  /**
   * Optional registry of live descendant sessions, owned by the conversation. A pause or stop
   * aborts the root request; registering children here lets the adapter end a worker session that
   * would otherwise keep running (and spending budget) after its parent turn was cancelled.
   */
  ...
  let totalCost = 0
  return (value: unknown): RuntimeEvent | undefined => {
    const input = object(value)
    if (input?.type === "session.created") {
      const info = object(object(input.properties)?.info)
      if (typeof info?.id === "string" && typeof info.parentID === "string" && sessions.has(info.parentID)) {
        sessions.add(info.id)
        descendants.add(info.id)
      }
      return undefined
    }
    const event = normalizeOpenCodeRuntimeEvent(value, state)
    if (!event || !sessions.has(event.sessionID)) return undefined
    if (event.sessionID !== rootSessionID) {
      if (event.type !== "step-finish") return undefined
      totalCost += event.cost
      return { ...event, sessionID: rootSessionID, cost: totalCost }
    }
    if (event.type === "step-finish") {
      totalCost += event.cost
      return { ...event, cost: totalCost }
    }
    return event
  }
}
  ...
  const conversation = (directory: string, id: string): import("./runtime-contract.ts").RuntimeConversation => {
    ...
    return {
      ...
      async events(signal) {
        ...
        const routeEvent = createOpenCodeConversationEventRouter(id, descendants)
        return {
          async *[Symbol.asyncIterator]() {
            for await (const event of stream) {
              const normalized = routeEvent(event)
      ...
      async abort() {
        // Children are aborted first and best-effort: the root abort below keeps its original
        // error contract, while a running worker no longer survives a pause unnoticed.
        await Promise.all(
          [...descendants].map((sessionID) =>
...
async function installRuntime(
  ...
  const directory = path.join(boomHomeDirectory(), "runtime")
...
export async function inspectRuntime() {
  ...
  try {
    ...
    if (
      ...
      !agents.data?.some((agent) => agent.name === "boom-pentest") ||
      !agents.data?.some((agent) => agent.name === "boom-flag-hunt") ||
      !agents.data?.some((agent) => agent.name === "boom-pentest-worker")
    )
      throw new Error("Boom's solver, worker, consultant, pentest, Flag-hunt, and pentest-worker agents were not registered by Boom's runtime.")
    if (
      tools.error ||
      !tools.data?.includes("ctf-note") ||
      !tools.data?.includes("ctf-submit") ||
      !tools.data?.includes("boom-exec") ||
      !tools.data?.includes("pentest-finding") ||
      !tools.data?.includes("pentest-flag")
    )
